...
from scripts.utils.run_utils import color2num, colorize
...
class CostModel:
    def __init__(self, env_wrap, obs_size = None, action_size = None, idx_goal = None, config=default_config):
        super().__init__()
        self.env_wrap = env_wrap
        self.idx_goal = idx_goal if idx_goal else env_wrap.key_to_slice["goal"]
        self.state_dim = obs_size if obs_size else env_wrap.observation_size
        self.action_dim = action_size if action_size else env_wrap.action_size
